...
def HierFAVG(args):
    ...
    # Build dataloaders
    train_loaders, test_loaders, v_train_loader, v_test_loader = get_dataloaders(args)  # TODO figure out what this does
    ...
    for i in range(args.num_clients):
        clients.append(Client(id=i,
                              train_loader=train_loaders[i],  # TODO how do we get the data loaders?
                              test_loader=test_loaders[i],
                              args=args,
                              device=device)
                       )

    # Set all client models to same parameter values
    initilize_parameters = list(clients[0].model.shared_layers.parameters())  # NOTE: If --global_model, all layers are shared
    ...
    # Create edge servers and assign clients
    edges = []
    cids = np.arange(args.num_clients)
    clients_per_edge = int(args.num_clients / args.num_edges)  # TODO num_clients should be divisible by num_edges? What if not uniform
    p_clients = [0.0] * args.num_edges

    if args.iid == -2:  # TODO figure this out
        if args.edgeiid == 1:
    ...
    # Create cloud server
    cloud = Cloud(shared_layers=copy.deepcopy(clients[0].model.shared_layers))  # shared_layers param not being used
    ...
    for num_comm in tqdm(range(args.num_communication)):
        ...
        # Prepare cloud server for a round
        cloud.refresh_cloudserver()

        # Signup all edges for cloud training round
        for edge in edges:
